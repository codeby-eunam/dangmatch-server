...
export default function YnuTournamentPage() {
  ...
  return (
    <div className="h-screen relative overflow-hidden" style={{ fontFamily: 'Arial, sans-serif' }}>
      ...
      <div className="h-full flex flex-col md:flex-row">
        ...
        >
          {r1.images?.[0] && (
            <div
              ...
              style={{ backgroundImage: `url(${r1.images[0]})`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.35 }}
            />
          )}
          {!r1.images?.[0] && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none" style={{ opacity: 0.07 }}>
          ...
          <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200" style={{ background: 'rgba(255,255,255,0.07)' }} />
          ...
          <div className="relative z-10 text-center px-6 py-20">
            {r1.isBye ? (
              ...
              <>
                <p className="text-xs font-black tracking-widest uppercase mb-4" style={{ color: '#FFD580', opacity: 0.9 }}>
                  {r1.category.split('>').pop()?.trim()}
                </p>
                <h2 className="font-black text-white mb-8 leading-tight" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', textShadow: '2px 2px 8px rgba(0,0,0,0.5)' }}>
                  {r1.name}
                </h2>
                <div className="inline-flex items-center gap-2 px-7 py-3 font-black text-sm text-white" style={{ border: '2px solid rgba(255,255,255,0.8)', borderRadius: 2 }}>
                  투표하기 👆
                </div>
              </>
            )}
        ...
        >
          {r2.images?.[0] && (
            <div
              className="absolute inset-0"
              style={{ backgroundImage: `url(${r2.images[0]})`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.35 }}
            />
          )}
          {!r2.images?.[0] && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none" style={{ opacity: 0.07 }}>
          ...
          <div className="relative z-10 text-center px-6 py-20">
            {r2.isBye ? (
              ...
              <>
                <p className="text-xs font-black tracking-widest uppercase mb-4" style={{ color: '#A8FFB8', opacity: 0.9 }}>
      ...
      {/* ── VS 배지 (모바일) ── */}
      <div className="md:hidden absolute inset-x-0 z-20 flex flex-row items-center justify-center pointer-events-none" style={{ top: '50%', transform: 'translateY(-50%)', height: 64 }}>
        <div className="flex-1" style={{ height: 2, background: 'rgba(255,255,255,0.2)' }} />
        <div className="flex items-center justify-center font-black text-sm flex-shrink-0" style={{ width: 52, height: 52, background: '#1A1A1A', color: '#FF9900', borderRadius: '50%', border: '3px solid #FF9900', boxShadow: '0 0 24px rgba(255,153,0,0.5), 0 4px 12px rgba(0,0,0,0.5)' }}>
          VS
        </div>
        <div className="flex-1" style={{ height: 2, background: 'rgba(255,255,255,0.2)' }} />
      </div>
      ...
      {/* ── VS 배지 (웹) ── */}
      <div className="hidden md:flex absolute inset-y-0 z-20 flex-col items-center justify-center pointer-events-none" style={{ left: '50%', transform: 'translateX(-50%)', width: 64 }}>
      ...
      <div className="absolute top-0 left-0 right-0 z-30 flex items-center justify-between px-4 py-3">
        <button
          ...
          style={{ background: 'rgba(0,0,0,0.55)', color: '#FFFFFF', backdropFilter: 'blur(6px)', borderRadius: 4 }}
        >
          ← 대진표
        </button>
        <div className="text-xs font-black tracking-wider px-3 py-2" style={{ background: 'rgba(0,0,0,0.55)', color: '#FFFFFF', backdropFilter: 'blur(6px)', borderRadius: 4 }}>
          토너먼트 {getRoundName()}
        </div>
        <button
          onClick={() => router.push('/yeungnam')}
      ...
      {/* ── 하단 바 ── */}
      <div className="absolute bottom-0 left-0 right-0 z-30 flex items-center justify-between px-6 py-3" style={{ background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)' }}>
        <div className="flex items-center gap-2">
          <div className="flex -space-x-1">
            {[1, 2, 3].map((i) => (
              <div key={i} className="w-5 h-5 rounded-full flex-shrink-0" style={{ background: `hsl(${i * 80}, 60%, 55%)`, border: '1.5px solid #222' }} />
            ))}
          </div>
          <span className="text-xs" style={{ color: 'rgba(255,255,255,0.6)' }}>{currentMatchIndex + 1}/{totalMatches} 경기</span>
        </div>
        <div className="text-xs font-black px-3 py-1" style={{ background: '#FF9900', color: '#1A1A1A', borderRadius: 2 }}>
          {getRoundName()}
        </div>
        <span className="text-xs font-bold" style={{ color: 'rgba(255,255,255,0.6)' }}>{nextRoundWinners.length}승 진출</span>
      </div>
    </div>
  );
}
