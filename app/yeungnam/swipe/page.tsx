...
const SwipeCard = forwardRef<SwipeCardRef, SwipeCardProps>(
  ({ restaurant, onSwipe, stackIndex, disabled }, ref) => {
    ...
    const doSwipe = async (dir: 'left' | 'right') => {
      await animate(x, dir === 'right' ? 600 : -600, { duration: 0.28, ease: 'easeOut' });
      onSwipe(dir);
    };
    ...
    return (
      <motion.div
        animate={{ scale: 1 - stackIndex * 0.05, y: stackIndex * 14 }}
        transition={{ type: 'spring', stiffness: 300, damping: 28 }}
        style={{ x: isTop ? x : 0, rotate: isTop ? rotate : 0, zIndex: 10 - stackIndex, position: 'absolute', width: '100%', touchAction: 'none' }}
      ...
      >
        {isTop && (
          <motion.div style={{ opacity: pickOpacity, border: '3px solid #FF9900', color: '#FF9900' }} className="absolute top-5 left-4 z-20 font-black text-lg px-3 py-1 select-none pointer-events-none -rotate-12">
            PICK
          </motion.div>
        )}
        {isTop && (
          <motion.div style={{ opacity: skipOpacity, border: '3px solid #8C8C8C', color: '#8C8C8C' }} className="absolute top-5 right-4 z-20 font-black text-lg px-3 py-1 select-none pointer-events-none rotate-12">
            SKIP
          </motion.div>
        )}

        <div className="overflow-hidden mx-1 select-none" style={{ background: '#FFFFFF', border: '2px solid #E8DDB8', boxShadow: isTop ? '6px 6px 0 rgba(0,0,0,0.10)' : '3px 3px 0 rgba(0,0,0,0.06)' }}>
          <div className="px-4 pt-3 pb-2 flex items-center justify-between">
            <span className="text-xs font-black tracking-widest uppercase px-2 py-0.5" style={{ background: '#FF9900', color: '#FFFFFF' }}>
              SEASON IN COLLECTION
            </span>
            <span className="text-xs" style={{ color: '#8C8C8C' }}>{getDistanceText(restaurant.distance)}</span>
          </div>
          <div className="w-full overflow-hidden relative" style={{ height: 220, background: '#F5EDD0' }}>
            {restaurant.images?.[0] ? (
              <img src={restaurant.images[0]} alt={restaurant.name} className="w-full h-full object-cover" draggable={false} />
            ) : (
              <div className="w-full h-full flex items-center justify-center">
                <span className="text-8xl">{getCategoryEmoji(restaurant.category)}</span>
              </div>
            )}
            <div className="absolute top-3 right-3 text-xs font-black px-2 py-0.5" style={{ background: '#1C8B40', color: '#FFFFFF' }}>
              PICK
            </div>
          </div>
          <div className="px-4 py-4">
            <h2 className="text-xl font-black mb-1 truncate" style={{ color: '#1A1A1A' }}>{restaurant.name}</h2>
            <p className="text-xs mb-3 font-bold italic" style={{ color: '#FF9900' }}>{restaurant.category.split('>').pop()?.trim()}</p>
            <p className="text-xs leading-relaxed" style={{ color: '#8C8C8C' }}>{restaurant.address}</p>
          </div>
        </div>
      </motion.div>
    );
...
export default function YnuSwipePage() {
  ...
  useEffect(() => {
    ...
    upsertRestaurants(restaurants).catch((err) => console.error('[Firestore upsert 실패]', err));
  }, [restaurants, router]);
  ...
  const handleSwipe = (dir: 'left' | 'right') => {
    ...
    if (dir === 'right' && restaurant) {
      ...
      // 영남대 피드에 PICK 기록
      if (user?.uid) {
        recordSchoolLike(YNU_DOMAIN, user.uid, restaurant);
      }
    }
  ...
  const startTournament = () => {
    ...
    router.push('/yeungnam/tournament');
  };
    ...
    <div className="min-h-screen flex flex-col" style={{ background: '#F5EDD0' }}>
      ...
      <div className="px-6 pt-4 pb-2 flex items-center justify-between">
        ...
        <div className="text-right">
          <div className="inline-block px-3 py-1.5 font-black text-sm" style={{ background: '#1A1A1A', color: '#FFFFFF' }}>
      ...
      <div className="flex-1 flex items-start justify-center px-4 pt-2">
        {allSwiped ? (
          <div className="text-center mt-12">
            <div className="text-5xl mb-4 font-black" style={{ color: '#FF9900' }}>✓</div>
            ...
            {redirectCountdown !== null && (
              <p className="mt-4 text-xs font-bold" style={{ color: '#8C8C8C' }}>
                {redirectCountdown}초 후 영남대 홈으로 돌아갑니다
              </p>
            )}
          </div>
        ) : (
      ...
      <div className="px-6 pb-6 pt-4">
        ...
        <div className="mb-4">
          ...
          <div className="h-2 overflow-hidden" style={{ background: '#E8DDB8', borderRadius: 2 }}>
            <div className="h-2 transition-all duration-300" style={{ width: `${progressPercent}%`, background: '#FF9900', borderRadius: 2 }} />
          </div>
        </div>
        ...
        <button
          ...
          style={{ background: canStart ? '#1A1A1A' : '#E8DDB8', color: canStart ? '#FFFFFF' : '#8C8C8C', borderRadius: 2 }}
        >
          {selectedCount === 0
            ? '아직 선택한 식당이 없어요'
            : selectedCount === 1
